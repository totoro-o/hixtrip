...
public class PayFailHandler implements PayHandler{
    ...
    public void doHandler(CommandPay commandPay) {
        ...
        if (canHandler(commandPay)){
            ...
            //修改库存
            inventoryDomainService.changeInventory(order.getSkuId(),Long.valueOf(order.getAmount()), - Long.valueOf(order.getAmount()), 0L);
        }else {
    ...
    public Boolean canHandler(CommandPay commandPay) {
        return PayStatusEnum.FAIL.getValue().equals(commandPay.getPayStatus());
    }
